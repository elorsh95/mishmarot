...
**הגדרת המייל נעשית אוטומטית בפריסה** (`npm run configure-auth`, נקרא מ-CI): הדומיין של האתר נוסף ל-Authorized domains, הקישור במייל מפנה ל-`https://<כתובת-האתר>/auth/action`, ונושא המייל ותוכנו מוגדרים בעברית. הנוסח נמצא ב-`scripts/configure-auth.ts`. הוא נשלח גם בהזמנה וגם באיפוס סיסמה, ולכן מנוסח כך שיתאים לשני המקרים.
