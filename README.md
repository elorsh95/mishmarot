...
**הגדרת המייל נעשית אוטומטית בפריסה** (`npm run configure-auth`, נקרא מ-CI): הדומיין של האתר נוסף ל-Authorized domains, והקישור במייל מפנה ל-`https://<כתובת-האתר>/auth/action`. נוסח המייל הוא התבנית המובנית של Firebase בעברית: Firebase לא מאפשר לשנות אותו בפרויקט הזה (`EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`). אותו מייל נשלח גם בהזמנה וגם באיפוס סיסמה.
